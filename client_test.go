...
func TestClient_WithOptions(t *testing.T) {
	tests := []struct {
		...
		{
			name: "with max turns",
			opts: []AgentOption{WithMaxTurns(10)},
		},
		{
...
func TestClient_WithOptionsVerification(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			opts: []AgentOption{WithIncludePartialMessages(true)},
		},
		{
			name: "with max budget",
			opts: []AgentOption{WithMaxBudgetUSD(1.0)},
		},
		{
		...
		{
			name: "with betas",
			opts: []AgentOption{WithBetas(SdkBetaContext1M)},
		},
		{
			name: "with setting sources",
			opts: []AgentOption{WithSettingSources(SettingSourceUser, SettingSourceProject)},
		},
		{
		...
		},
		{
			...
			opts: []AgentOption{WithEnableFileCheckpointing(true)},
		},
		{
			name: "with add dirs",
