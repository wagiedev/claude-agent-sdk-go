...
func QueryStream(
	...
	return func(yield func(Message, error) bool) {
		...
		// Check if MCP servers or hooks are present - if so, we need to wait for result
		// before closing stdin to allow tool calls to complete
		hasMCPOrHooks := len(options.MCPServers) > 0 || len(options.Hooks) > 0
